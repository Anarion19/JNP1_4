...
private:
    ...
    template <typename U>
    void countImperial(ImperialStarship<U> ship) {
        (void) ship;
        if(ship.getShield() > 0) impShips++;
    }
    template <typename U>
    void countImperial(U ship) {
        (void) ship;
    }
    template <typename U>
    void countRebel(ImperialStarship<U> ship) {
        (void) ship;
    }
    template <typename U>
    void countRebel(U ship) {
        (void) ship;
        if(ship.getShield() > 0) rebShips++;
    }
    template <typename I>
    void printShield(I ship) {
        std::cout<<ship.getShield() << std::endl;
    }
    void printState() {
        std::apply([&](auto ...x){(..., printShield(x));}, fleet);
    }
...
public:
    ...
    size_t countImperialFleet() {
        impShips = 0;
        std::apply([&](auto ...x){(..., countImperial(x));}, fleet);
        return impShips;
    }
    size_t countRebelFleet() {
        rebShips = 0;
        std::apply([&](auto ...x){(..., countRebel(x));}, fleet);
        return rebShips;
    }
